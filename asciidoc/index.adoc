...
public static class RebillJob implements DistributedJob {
    ...
    public class CustomAssignmentStrategy implements AssignmentStrategy {
        ...
        public AssignmentState reassignAndBalance(
            ...
            Map<JobId, List<WorkItem>> itemsToRebillJob = Collections.singletonMap(
                new JobId("rebill-job"), itemsToAssign.get(new JobId("rebill-job"))
            );
            ...
            Map<JobId, List<WorkItem>> itemsToUssdJob = Collections.singletonMap(
                new JobId("ussd-job"), itemsToAssign.get(new JobId("ussd-job"))
            );
            // here we implements custom logic for ussd job distribution...
            ...
            ...
            Map<JobId, List<WorkItem>> itemsToSmsJob = Collections.singletonMap(
                new JobId("sms-job"), itemsToAssign.get(new JobId("sms-job"))
            );
            // here we implements custom logic for sms job distribution...
            ...
            // remove sms job from itemsToAssign
            itemsToAssign.remove(new JobId("ussd-job"));

            // reassign rest items to assign
