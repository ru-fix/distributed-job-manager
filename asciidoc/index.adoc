...
Manager also removes not relevant (which isn't in any `workers/*/available` subtree) jobs from `work-pool` subtree using `work-pool-version` lock (clearing process) +
