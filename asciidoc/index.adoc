...
class MyJob implements DistributedJob {
    ...
    public MyJob(AssignmentStrategy strategy) {
...
Assignment strategy is the behavior of how the distribution of work items by workers will change,
when number active workers changes. +
DJM provides some implementations for the most common cases of distribution.
Also you can implement AssigmentStrategy for your case by himself and inject it to your distributed job.

* ClassifiedAssignmentStrategy +
Intended for distribution classified (grouped by type) jobs,
so one worker can't handle more than one job of the same class
* DefaultAssignmentStrategy
* LexicographicalAssignmentStrategy +
Ignore current workers assignment. Assign jobs to workers and try balance loading uniformly
* MinimizedReassignmentAssignmentStrategy +
It minimize job reassignment within
living workers when new worker added or one of workers removed
* SingletonAssignmentStrategy +
Ensures that with repeated distributions, one job will be launched on only one worker
* FixedSizeWorkersAssignmentStrategy
Ensures that with repeated distributions, worker will handle one work item of one job.
...
class EmailSenderJob implements DistributedJob {
    private String jobId;
    private AssignmentStrategy strategy;

    public EmailSenderJob(AssignmentStrategy strategy) {
        this.strategy = strategy;
    }

    WorkPool getWorkPool(){
        return WorkPool.of(new HashSet<>(Arrays.asList("sendEmail1", "sendEmail2")));
    }
...
DistributedJobManager distributedJobManager(
        ...
        StatisticsLogKafkaToHBaseExporterJob statisticsLogKafkaToHBaseExportJob,
        ElasticsearchStatisticsUploadJob elasticsearchStatisticsUploadJob,
        TotalSpentMoneyUpdateJob totalSpentMoneyUpdateJob
) throws Exception {
    return new DistributedJobManager(
            "my-app-1", // application id
            CuratorFrameworkFactory.newClient(zkHosts, new ExponentialBackoffRetry(1000, 10)),
            zkRoot + "/job-manager",
            Arrays.asList(
                    emailSenderJob,
                    statisticsLogKafkaToHBaseExportJob,
                    elasticsearchStatisticsUploadJob,
                    totalSpentMoneyUpdateJob
            ),
...
@Bean
EmailSenderJob emailSenderJob() {
    return new EmailSenderJob("email.sender.job", new SingletonAssignmentStrategy());
}
//...
----
